...
cat("Structure of the empty Data Frame:\n")
str(df)
```
#Output
```The output shows that the data frame has 0 observation (rows) and 5 variables (columns) with their respective data types. The "Factors" column is empty since there are no levels defined yet.  
```
#NUMBER 3
``` {r number3}
household_data <- data.frame (
  ...
  Types_of_Houses = c("Wood","Congcrete", "Congcrete", "Wood","Semi-Crongcrete", "Semi-Congcrete", "Wood", "Semi-Congcrete", "Semi-Congcrete", "Congcrete")
)
household_data
...
household_data$Types_of_Houses <- as.integer(factor(household_data$Types_of_Houses,
                  levels = c("Wood", "Congcrete", "Semi-Congcrete"),
                  labels = c(1,2,3)))
household_data
```
#D
...
```It is a Sentiment Analysis over time on Twiiter data, specifically for the dates July 14, 15, 17, 18, 20, and 21 in the year 2020. The Sentiment analysis categorizes tweets into three sentiment groups: negative, neutral, and positive. Here's an interpretation of the data;
...
-July 17, 2020:
-Negative count: Approximately 3,250 tweets were categorized as negative.
...
-July 18, 2020:
-Negative count: There were still around 3,250 tweets were categorized as negative.
-Neutral count: Around 2,000 tweets were categorized as neutral. 
-Positive count: Approximately 2,500 tweets were categorized as positive. 

-July 20, 2020:
-Negative count: Nearly 2,300 tweets were categorized as negative.
-Neutral count: Around 1,450 tweets were categorized as neutral. 
-Positive count: Approximately 1,750 tweets were categorized as positive. 

-July 21, 2020:
-Negative count: Around 4,050 tweets were categorized as negative.
-Neutral count: Around 2,250 tweets were categorized as neutral. 
-Positive count: Approximately 2,900 tweets were categorized as positive. 
```
